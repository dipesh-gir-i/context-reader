...
function Popup() {
  ...
  return <div style={{width:320,padding:18,background:'var(--surface)'}}><div className="brand" style={{padding:'0 0 14px'}}><div className="brand-mark">CR</div><div><div className="brand-name">Context Reader</div><div className="sidebar-sub" style={{color:'var(--muted)'}}>Read. Notice. Understand.</div></div></div><p style={{fontSize:13,lineHeight:1.5,color:'var(--muted)',margin:'4px 0 17px'}}>Open a PDF in the reader, then highlight any word to get its meaning in context.</p><button className="primary-btn" style={{width:'100%',marginBottom:8}} onClick={openReader}><Icon name="book" size={15}/> Open Reader</button><button className="ghost-btn" style={{width:'100%'}} onClick={openSettings}><Icon name="settings" size={14}/> Settings & API key</button></div>;
}
